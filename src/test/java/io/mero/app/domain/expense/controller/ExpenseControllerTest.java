...
class ExpenseControllerTest {
    ...
    void linkFootprint_Success() throws Exception {
        ...
        ExpenseResponse response = new ExpenseResponse(
                ...
                new BigDecimal("150"), Currency.USD,
                1L, "식비", "🍔", "#FF5733",
                "스타벅스", LocalDate.now(), "도쿄",
                LocalDateTime.now()
        );

        given(expenseService.linkFootprint(anyLong(), eq(1L), eq(10L)))
                .willReturn(response);
    ...
    @Test
    @DisplayName("Footprint 연결 해제 API 성공")
    void unlinkFootprint_Success() throws Exception {
        // given
        ExpenseResponse response = new ExpenseResponse(
        ...
        given(expenseService.unlinkFootprint(anyLong(), eq(1L)))
                .willReturn(response);

        // when & then
        mockMvc.perform(delete("/api/trips/1/expenses/1/footprint"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.footprintId").isEmpty());
    }
